    ...
    <header className="sticky top-0 z-20 flex h-16 items-center gap-3 border-b border-slate-200 bg-white/90 px-4 backdrop-blur-md lg:px-6">
      ...
      <div className="flex items-center gap-2">
        <div className="relative">
          ...
          {notificationsOpen && (
            <div className={`dropdown-panel absolute right-0 top-12 z-50 w-[min(24rem,calc(100vw-2rem))] overflow-hidden rounded-xl border border-slate-200 bg-white shadow-elevated ${notificationsClosing ? 'dropdown-panel-closing' : 'dropdown-panel-open'}`}>
              ...
              <div className="max-h-[min(28rem,calc(100vh-8rem))] overflow-y-auto scrollbar-thin">
                {notifications.map((notification) => <button key={notification.id} type="button" onClick={() => { setNotifications((current) => current.map((item) => item.id === notification.id ? { ...item, unread: false } : item)); closeNotifications(); }} className={`flex w-full gap-3 border-b border-slate-50 px-4 py-3 text-left transition hover:bg-emerald-50 ${notification.unread ? 'bg-brand-50/40' : 'bg-white'}`}><span className={`mt-1.5 h-2 w-2 shrink-0 rounded-full ${notification.unread ? 'bg-brand-500' : 'bg-slate-200'}`} /><span className="min-w-0"><span className="block text-xs font-bold text-slate-700">{notification.title}</span><span className="mt-0.5 block text-xs leading-5 text-slate-500">{notification.message}</span><span className="mt-1 block text-[10px] font-medium text-slate-400">{notification.time}</span></span></button>)}
              </div>
            </div>
          )}
        </div>
